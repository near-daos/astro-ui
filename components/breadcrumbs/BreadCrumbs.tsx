...
export const BreadCrumbs: React.FC<BreadCrumbsProps> = ({ locationPath, testString }) => {
  const locationsList = locationPath.split('/').filter(item => item !== '');
  ...
  return (
    <>
      {testString && (
        <span className={styles.breadcrumbsTest}>{testString}</span>
      )}
